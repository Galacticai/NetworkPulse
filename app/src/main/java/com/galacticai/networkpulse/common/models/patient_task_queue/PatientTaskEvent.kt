...
sealed interface PatientTaskEvent {
    ...
    data class TaskStop<K>(val key: K, val startedAt: Date, val runtime: Duration) : PatientTaskEvent
    data class TaskDone<K, V>(
        val key: K, val value: V?, val startedAt: Date, val runtime: Duration
    ) : PatientTaskEvent

    data class TaskTimeout<K>(val key: K, val timeout: Duration, val startedAt: Date) : PatientTaskEvent
